...
> **Amended on 2026-08-28. A FOURTH KEY, `gt-jira-cart.presets`, and it is on the
> COLLECTIONS side of the line this section draws.** Nothing above changes. The key
> holds two lists — one for 📋 Details, one for 📊 Report — of the export
> configuration under names the user typed:
>
> ```json
> {
>   "details": [
>     { "id": "0f3a…", "name": "Standard", "star": true,
>       "lineShape": "markdown",
>       "fields": [ { "id": "type", "on": true } ] }
>   ],
>   "report": [
>     { "id": "9c22…", "name": "Standard", "star": true,
>       "lineShape": "markdown",
...
> ```
>
> **The one rule that is new in kind: this key is REPAIRED PER ENTRY, where the
> preferences key is REPLACED WHOLESALE — and the reason is which side of the line
> above the data sits on.** A preference that does not parse falls back to the
> shipped defaults, because a preference is regenerated by clicking a switch. A
> preset is not: it is the fields, their order, the line shape and the two headings
> that somebody built and named, so a preset whose field list is rubbish gets **that
> list** repaired, a preset with no usable name is dropped, and the rest survive. The
> whole-blob fallback the preferences take would read here as *you lost every preset
> you built*, and it would be a designed behaviour rather than an accident. It is the
> same argument the migration table's last row makes about a collection, arriving at
> a gentler answer because a preset **can** be repaired where a damaged collection
> cannot: an item this build cannot read might be an issue key, and inventing one
> would be data loss, but a band this build cannot read has a default that is a real
> configuration and nothing is lost by using it.
>
> **None of this goes near `load`.** That function is the collections path and
> carries the four migration rows; this is a fourth key with its own reader.
>
> Details settled by implication, recorded so nobody decides them again. A preset's
> `id` is opaque, `crypto.randomUUID()`, generated once and never derived from the
> name, so a rename is free — this section's own reasoning for a collection's id.
> **★ is a flag on the preset and not an id stored elsewhere**, which is this
> section's *there is no active pointer* one step across: `collections` keeps no
> pointer to the active one because it is `collections[0]`, and presets cannot use
> position because they are displayed sorted by name, so the flag is what replaces
> it. Delete the preset and the flag goes with it. **Exactly one ★ per list is
> enforced on read** — zero, two, or a non-boolean sends the flag to the first preset
> by name — because every screen that asks *what does a plain press print* has to get
> exactly one answer. **The array's order means nothing**: the picker sorts by name
> and ★ is a flag, so nothing reads position and nothing preserves it. `band1` and
> `band2` exist on a 📊 Report preset only, because 📋 Details has no headings and a
> key that existed and was never read would be a promise the format does not keep.
>
> **First run is the only migration, and there is no version and no flag.** If the
> key is absent, one preset per list is built, called `Standard`, carrying
> `detailsFields`, `reportFields`, `reportBand1`, `reportBand2` and `lineShape` as
> the preferences hold them **right now** — so both buttons print exactly what they
> printed at 1.6.0, byte for byte. **The key existing is what says the build has
> happened**: no flag, no version, no second value that could disagree, which is the
> same shape of choice as *there is no active pointer* above. `Standard` and not
> `Default`, because *the default preset* and *the preset called Default* would
> become two different things the moment ★ moved.
>
> **`v` is not bumped and there is no `.bak`.** No stored item changed shape; this is
> a new key, which by the rule below is never a migration.
>
> **The four export keys have NOT yet left the preferences, and that is a sequencing
> decision rather than an oversight.** `detailsFields`, `reportFields`, `reportBand1`
> and `reportBand2` become a preset's business and `lineShape` stays as 🔗 Links' own
> setting — but the two things that read those keys, `format` and the ⚙ panel, still
> read them, and nothing reads a preset yet. Dropping them alongside this key would
> have taken 📋 Details and 📊 Report out with them. They leave with the ticket that
> moves those readers onto the ★ preset. Until then the export configuration is
