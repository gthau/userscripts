 ...
 *   collections, and four copy buttons: 🔗 Links, 📃 Names, 🔑 Keys, 🔍 JQL.
...
(function () {
  ...
  // is only a view of it. A React remount must not close it, which is why the
  // state is a variable here and a rule keyed off <html>, and never a node React
  // owns (§7 steps 10 and 11).
...
${selectors.join(",\n")} {
  ...
  function ensureBadge() {
    ...
    badge.addEventListener("click", () =>
      guard(() => setDrawerOpen(!drawerOpen)),
    );
  ...
  function renderBadge(state) {
    ...
    let title = `The Cart: ${count} item${count === 1 ? "" : "s"} in ${collection.name}. Click to ${drawerOpen ? "close" : "open"} the drawer.`;
    let badgeState = "ok";
    ...
    badge.setAttribute("aria-expanded", String(drawerOpen));
  }
  ...
  // save and share. Its target is a plain search input, so no rich version (§2.8).
  ...
  const COPY_FORMATS = [
    ...
    },
    {
      kind: "jql",
      label: "🔍 JQL",
      title: "Copy key in (KEY, KEY, KEY) for Jira's own search box",
      build: formatJql,
  ...
  function format(kind, items, scope) {
    const entry = COPY_FORMATS.find((one) => one.kind === kind);
  ...
  function ensureDrawer() {
    ...
    foot.id = FOOT_ID;
    for (const spec of COPY_FORMATS) {
      const button = actionButton("gt-cart-copy", "copy");
  ...
  function setDrawerOpen(open) {
    ...
    // so there is no in-memory copy to refresh (§2.5, rule 3). Called directly
    // rather than scheduled, so the drawer is there in the same frame as the click.
    render();
  }
  ...
  function renderDrawer(state, scan) {
    ...
    // React remount can take away (§2.9, §2.11).
    if (!drawerOpen) return;
  ...
  function liveRow(row, collectionName) {
    const node = actionButton("gt-cart-row", "toggle-item");
    node.dataset.gtKey = row.key;
    ...
    node.title = summaryTooltip(row.key, row.summary, [
      row.collected
        ? `Click to remove it from ${collectionName}`
        : `Click to add it to ${collectionName}`,
    ]);

    node.append(el("span", "gt-cart-row-key", row.key));
    node.append(el("span", "gt-cart-row-summary", row.summary));
    if (row.origin) node.append(el("span", "gt-cart-row-origin", row.origin));
    ...
    node.append(el("span", "gt-cart-row-mark"));
    return node;
  }
  ...
  function renderFoot(state) {
    ...
    for (const spec of COPY_FORMATS) {
      ...
      // `jira-ux` already uses for the buttons that need a description -- and a
      // copy of zero items must not write at all (§2.8).
  ...
  function render() {
    ...
    const scan = scanPage(drawerOpen);
...
/* Open and closed is ONE attribute on html and one rule, the same lever the
   badge's corner uses. The state itself is a variable in memory: it answers "am I
   collecting right now", which is a question about this sitting rather than a
   standing preference, so a reload closes it and loses nothing -- the collection
   is in storage and the drawer is only a view of it (§2.9). */
...
${D} button.gt-cart-row:hover,
${D} div.gt-cart-item:hover {
  background: var(--gt-cart-hover);
}
${D} button.gt-cart-row:focus-visible,
...
${D} button.gt-cart-row[data-gt-collected="true"] {
  background: var(--gt-cart-collected-bg);
  color: var(--gt-cart-collected-text);
}
...
${D} button.gt-cart-row[data-gt-collected="false"] span.gt-cart-row-mark::after {
  content: "+";
}
${D} button.gt-cart-row[data-gt-collected="true"] span.gt-cart-row-mark::after {
  content: "✓";
}
/* The same pre-click warning the floating button gives: the row turns red and
   names removal BEFORE any click, because removal is the one destructive thing
   these gestures do and there is no undo (§2.9, §2.7). */
${D} button.gt-cart-row[data-gt-collected="true"]:hover {
  background: var(--gt-cart-remove);
  color: var(--gt-cart-on-bold);
}
${D} button.gt-cart-row[data-gt-collected="true"]:hover span.gt-cart-row-mark::after {
  content: "−";
}
...
div#${MENU_ID} button.gt-cart-menu-item:hover {
  ...
  guard(() => {
    ...
    root.dataset.gtCartCorner = prefs.corner;
    root.dataset.gtCartLayout = prefs.layout;
    // The drawer starts closed, so the rule that hides it is correct before the
    // element it hides exists (§2.9).
  ...
  // Jira's dialogs would read as a bug (§2.9). Nothing below touches `drawerOpen`.
  document.addEventListener(
    "pointerdown",
    (event) =>
      guard(() => {
