...
//
//     node test/jira-backlog-sprints/run.mjs
//
...
//
// The same loop as `test/jira-cart/run.mjs`, copied rather than shared, for the
// reason ADR §2.13 gives about the duplicated helpers: a thing that can break
// between you and the answer is not worth the lines it saves.
//
// A harness here needs a browser, and one of them needs a network the first time.
// Either can decide it cannot run, in which case it prints a SKIP line and reports
// zero checks. Zero checks is the signal to read the line -- it is not a pass.
//
...
for (const name of harnesses) {
  ...
  console.log(`${passed ? "ok  " : "FAIL"} ${name.padEnd(20)} ${String(ok).padStart(3)} checks${bad ? `, ${bad} failed` : ""}`);
}
