...
**451 checks across seven files. No framework, no `package.json`, no dependencies.**
...
| `format-smoke.mjs` | 131 | The **five** copy formats against §2.8's and §2.14's worked examples, `bulkfetch` response validation, `uniqueName`, and every failure sentence §2.9's table promises, word for word. Since 1.1.0 it also asserts the four rules §2.14 bought with real pastes — no `opacity`, no inline `border`, no separator that is a box, no colour without a pale ground |
| `boot-smoke.mjs` | 154 | **The whole script**, against a fake DOM, driven by real clicks through the delegated listeners it really uses. Since 1.1.0 that includes 📋 Details' two presses, its expiry, and its refusal to arm on a refused fetch |
